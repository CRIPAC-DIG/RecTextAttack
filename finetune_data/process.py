...
parser.add_argument('--file_path', default='reviews_Sports_and_Outdoors_5.json.gz', help='Processing file path (.gz file).')
parser.add_argument('--meta_file_path', default='meta_Sports_and_Outdoors.json.gz', help='Processing file path (.gz file).')
parser.add_argument('--output_path', default='sports', help='Output directory')
args = parser.parse_args()
# Recformer uses Amazon v2
...
def extract_meta_data(path):
    ...
    with gzip.open(path) as f:
        for line in tqdm(f):
            ...
            attr_dict = dict()
            asin = line['asin']
            category = ' '.join(line['categories'][0]) if 'categories' in line else ''
