...
def encode_all_items(model: RecformerModel, tokenizer: RecformerTokenizer, tokenized_items, args): # individual items
...
def main():
    ...
    # path and file
    parser.add_argument('--pretrain_ckpt', type=str, default='pretrain_ckpt/recformer_seqrec_ckpt.bin')
    parser.add_argument('--data_path', type=str, default='finetune_data/beauty')
    ...
    dataset_name = args.data_path.split('/')[-1].split('_')[0]
    target_ids = json.load(open(f"./results/{args.attack}.{dataset_name}.ft.json"))
    target_ids = [int(i) for i in list(target_ids.keys())]
    attacked_items = target_ids
    print(target_ids[:10], len(target_ids))
    ...
    item_embeddings = torch.load(path_item_embeddings, map_location=args.device)
    model.init_item_embedding(item_embeddings)

    if os.path.exists(path_ckpt):
        print(f'Continue training from {path_ckpt}')
        model.load_state_dict(torch.load(path_ckpt))
        
    model.to(args.device) # send item embeddings to device
    if args.test_only:
        print('Test with the best checkpoint.', path_ckpt)
        model.load_state_dict(torch.load(path_ckpt))
        test_metrics, user_embeddings = eval(model, test_loader, args, attacked_items, return_user_embed=True)
        from prompt_attack.goal_function import target_item_exposure
        print(f'Test set: {test_metrics}')
        print('Target item exposure:', np.mean(target_item_exposure(user_embeddings, item_embeddings, attacked_items)))
        exit()
    ...
    for epoch in range(args.num_train_epochs):
        ...
        item_embeddings = encode_all_items(model.longformer, tokenizer, tokenized_items, args)
        model.init_item_embedding(item_embeddings) # update item embeddings 

        train_one_epoch(model, train_loader, optimizer, scheduler, scaler, args)
        
        if (epoch + 1) % args.verbose == 0:
            dev_metrics = eval(model, dev_loader, args, attacked_items)
